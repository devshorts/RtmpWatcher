...
namespace com.TheSilentGroup.Fluorine.SystemHelpers.Threading
{
	/// <summary>Implementation of Dijkstra's PV Semaphore based on the Monitor class.</summary>
	sealed class SemaphoreEx
	{
	...
	/// Managed threadpool.
	/// </summary>
	sealed class ThreadPoolEx : DisposableBase
	{
		...
		/// an auto reset event.
		/// </summary>
		private static SemaphoreEx _workerThreadNeeded;
		...
		static ThreadPoolEx() { Initialize(); }
		...
		private static void Initialize()
		{
			...
			_workerThreadNeeded = new SemaphoreEx(0);
			...
			for(int i=0; i<_maxWorkerThreads; i++)
			{
				...
				newThread.Name = "ThreadPoolEx #" + i.ToString();
				...
				if (callback != null)
				{
					...
					catch(Exception ex)
					{
						try
						{
							...
							if (handler != null) handler(typeof(ThreadPoolEx), new UnhandledExceptionEventArgs(ex, false));
						}
						catch{}
					}
					finally
