...
namespace com.TheSilentGroup.Fluorine.Filter
{
	...
	class DeserializationFilter : AbstractFilter
	{
		...
		{
			AMFDeserializer deserializer = new AMFDeserializer(context.ApplicationContext.InputStream);
			deserializer.UseLegacyCollection = _useLegacyCollection;
			AMFMessage amfMessage = deserializer.ReadAMFMessage(context.ApplicationContext);
			context.AMFMessage = amfMessage;
			context.MessageOutput = new MessageOutput(amfMessage.Version);
			if( context.ApplicationContext != null && context.ApplicationContext.FailedAMFBodies.Length > 0 )
			{
				AMFBody[] failedAMFBodies = context.ApplicationContext.FailedAMFBodies;
