...
namespace com.TheSilentGroup.Fluorine.Gateway
{
	...
	public class FluorineHttpApplicationContext : IApplicationContext
	{
		...
		{
			ClassMappings classMappings = (ClassMappings)ConfigurationSettings.GetConfig("fluorine/classMappings");
			if( classMappings == null )
				classMappings = new ClassMappings();
			httpApplication.Context.Application["ClassMappings"] = classMappings;

			ServiceMap serviceMap = (ServiceMap)ConfigurationSettings.GetConfig("fluorine/services");
			httpApplication.Context.Application["ServiceMap"] = serviceMap;

			CacheMap cacheMap = (CacheMap)ConfigurationSettings.GetConfig("fluorine/cache");
			httpApplication.Context.Application["CacheMap"] = cacheMap;

			NameValueCollection importNamespaces = (NameValueCollection)ConfigurationSettings.GetConfig("fluorine/importNamespaces");
			httpApplication.Context.Application["ImportNamespaces"] = importNamespaces;

			Hashtable nullables = ConfigurationSettings.GetConfig("fluorine/nullable") as Hashtable;
			httpApplication.Context.Application["NullableValues"] = nullables;

			string tmp = ConfigurationSettings.AppSettings["wsdlGenerateProxyClasses"];
			...
			tmp = ConfigurationSettings.AppSettings["wsdlProxyNamespace"];
			...
			tmp = ConfigurationSettings.AppSettings["acceptNullValueTypes"];
			...
			tmp = ConfigurationSettings.AppSettings["remotingServiceAttribute"];
			if( tmp != null )
			{
				try 
				{
